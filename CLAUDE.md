...
All source lives in the `terminal_tc/` package. Five files, each with a single responsibility:

- **`terminal_tc/__init__.py`** — Package marker and `__version__`.
- **`terminal_tc/__main__.py`** — Enables `python -m terminal_tc`.
- **`terminal_tc/config.py`** — `AppConfig` dataclass, JSON persistence (`~/.config/artnet-timecode/config.json`), and `validate_config`. Imported by both other files.
